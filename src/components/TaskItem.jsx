...
export function TaskItem({ task, selected, onSelect, selectionMode, isFocused, isEditingProp, onEditEnd, onFilterClick }) {
    ...
    // Check for "contact:" under cursor
    const handleInput = async (e) => {
        ...
        // Look backwards from cursor to find "contact:"
    ...
    const selectContact = (contact) => {
        ...
        // Replace "contact:Que..." with "contact:Name tel:... mail:..."
        const prefix = val.substring(0, startOfToken);

        // Sanitization
        const safeName = contact.name.replace(/\s+/g, '_'); // Replace spaces with underscores for single-token compliance
        ...
        handleEdit(newVal); // Save immediately? Or keep editing? 
        // Let's keep editing but update ref
                ...
                {showSuggestions && suggestions.length > 0 && (
                    <div className="absolute left-4 top-full mt-1 w-64 bg-zinc-900 border border-zinc-700 rounded-lg shadow-xl z-50 max-h-48 overflow-y-auto">
                        ...
                        {suggestions.map((contact) => (
                            ...
                            >
                                {contact.photo ? (
                                    <img src={contact.photo} alt="" className="w-6 h-6 rounded-full" />
                                ) : (
                                ...
                                <div className="overflow-hidden">
                                    <div className="text-sm text-zinc-200 truncate">{contact.name}</div>
